...
export default async function MenusPage({
  ...
  <span aria-hidden="true" />
                ...
                <div className="flex flex-row gap-8">
                    <Link href="/order/take-out" className={tabClass("all")}>すべて</Link>
                    <Link href="/order/take-out?kind=set" className={tabClass("set")}>セット</Link>
                    <Link href="/order/take-out?kind=single" className={tabClass("single")}>単品</Link>
                </div>
                ...
                <ul className="grid grid-cols-2 gap-4">
                    {rows.map((menu) => (
                        ...
                        >
                            <Link href={`/order/take-out/${menu.id}`}>
                            <img
                                src={menu.image_url}
                                alt={menu.name}
